...
$LogFilePath = "C:\DyPerformance\logfile-$($TimeStamp).txt"
$RegistryBackupPath = "C:\DyPerformance\resources\regedits.bak"
$BCDBackupPath = "C:\DyPerformance\resources\backup.bcd"
        ...
        foreach ($Computer in $ComputerName) {
            ...
            try {
                ...
                $SysProperties = [ordered]@{
                    "DyPerformance Version"       = $CurrentVersion
                    "CPU"                  = $CleanCPUName
...
function Write-Windows10Warning {
    ...
    BEGIN {
        ...
        $lines = @(
            ...
            "If you choose to run DyPerformance on this Windows 10 machine regardless, you are assuming any liability for a damaged system or unbootable environment.",
            ...
            "`nIf you want a better chance of DyPerformance working properly, then I'd recommend upgrading to Windows 11 (namely the Pro for Workstations variant) and starting there.",
            ...
            "`nGeneral support and information for this page can be discussed through discord @dyvertigo, but only provided you have done your due diligence and have tried to prevent or fix any issues as a result of your usage."
        )
    }
...
function Write-LegacyWindowsWarning () {
    ...
    PROCESS {
        Write-ColorOutput -InputObject  "You're running an unsupported Windows version! $OSVersion is not supported by DyPerformance!" -ForegroundColor Red -HorizontalPad $($TerminalWindowWidth)
    }
}
...
function Write-RisksWarning {
    ...
    BEGIN {
        $lines = @( 
            ...
            "- You are not entitled to on-demand 24/7 support.",
            "`nGeneral support and information for this page can be discussed through discord @dyvertigo, but only provided you have done your due diligence and have tried to prevent or fix any issues as a result of your usage."
        )
...
function Set-EnableSystemRecovery {
    ...
    PROCESS {
        ...
        Read-CommandStatus "Checkpoint-Computer -Description 'DyPerformanceTweaks'" "created a restore point pre-DyPerformance"
    }
...
function Write-MainMenuStart {
    ...
    BEGIN {
        ...
        Write-LogEntry "Starting DyPerformance..."
        Clear-Host
        $host.ui.RawUI.WindowTitle = "DyPerformance $($CurrentVersion)"

        # ASCII Art for menu screen
        $asciiArt = @"
`7MM"""Yb.   `YMM'   `MM'`7MM"""Mq.
  MM    `Yb.   VMA   ,V    MM   `MM.
  MM     `Mb    VMA ,V     MM   ,M9
  MM      MM     VMMP      MMmmdM9
  MM     ,MP      MM       MM
...
"@
        ...
        Write-Host "Thank you for using DyPerformance" -ForegroundColor Green
        Write-Host "DM me on discord for any help. @dyvertigo" -ForegroundColor Gray
    }
    ...
    PROCESS {
        ...
        if ($WindowsVersion -eq 11) {
            Write-Host  "You're currently running $($OSVersion)! Nice, let's get started." -ForegroundColor Green
            Write-Host "`nOptions:`n" -ForegroundColor DarkGray
            if (Test-Path $RegistryBackupPath -PathType Leaf) {
                Write-Host "[1] Run DyPerformance                      [5] Revert Changes" -ForegroundColor Gray
            } else {
                Write-Host "[1] Run DyPerformance" -ForegroundColor Gray
            }
            Write-Host "[2] Generate System Report" -ForegroundColor Gray
            Write-Host "[3] Optimize PowerShell" -ForegroundColor Gray
            Write-Host "[4] Activate Windows" -ForegroundColor Gray
            ...
            elseif ($Choice -eq "2") {
                Clear-Host
                Write-LogEntry "Running a system report via System Report Generator."
